import {
  secp256k1
} from "./chunk-OBP74VKE.js";
import {
  Chi,
  HashMD,
  Maj,
  sha256
} from "./chunk-23E3XHBV.js";
import {
  ...
  validate,
  withResolvers
} from "./chunk-6LN7I7E4.js";
import {
  keccak_256
} from "./chunk-KFW4HRPQ.js";
import {
  formatAbiItem,
  parseAbiItem
...
function decodeEventLog(parameters) {
  ...
  const isUnnamed = inputs == null ? void 0 : inputs.some((x4) => !("name" in x4 && x4.name));
...
function encodeEventTopics(parameters) {
  ...
  if (args && "inputs" in abiItem) {
    ...
    const args_ = Array.isArray(args) ? args : Object.values(args).length > 0 ? (indexedInputs == null ? void 0 : indexedInputs.map((x4) => args[x4.name])) ?? [] : [];
    if (args_.length > 0) {
      topics = (indexedInputs == null ? void 0 : indexedInputs.map((param, i3) => {
        if (Array.isArray(args_[i3]))
          return args_[i3].map((_3, j4) => encodeArg({ param, value: args_[i3][j4] }));
  ...
  return logs.map((log) => {
    ...
    try {
      ...
      if (err instanceof DecodeLogDataMismatch || err instanceof DecodeLogTopicsMismatch) {
        ...
        isUnnamed = (_a = err.abiItem.inputs) == null ? void 0 : _a.some((x4) => !("name" in x4 && x4.name));
      }
...
async function recoverPublicKey({ hash, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-2N4QCC7C.js");
  const signature_ = (() => {
...
function blobsToCommitments(parameters) {
  ...
  return to2 === "bytes" ? commitments : commitments.map((x4) => bytesToHex(x4));
}
...
function blobsToProofs(parameters) {
  ...
  return to2 === "bytes" ? proofs : proofs.map((x4) => bytesToHex(x4));
}
...
function toBlobs(parameters) {
  ...
  return to2 === "bytes" ? blobs.map((x4) => x4.bytes) : blobs.map((x4) => bytesToHex(x4.bytes));
}
...
var SHA1 = class extends HashMD {
  ...
  get() {
    const { A: A2, B: B2, C: C3, D: D2, E: E4 } = this;
    return [A2, B2, C3, D2, E4];
  }
  set(A2, B2, C3, D2, E4) {
    this.A = A2 | 0;
    this.B = B2 | 0;
  ...
  process(view, offset) {
    ...
    let { A: A2, B: B2, C: C3, D: D2, E: E4 } = this;
    for (let i3 = 0; i3 < 80; i3++) {
      ...
      const T3 = rotl(A2, 5) + F + E4 + K4 + SHA1_W[i3] | 0;
      E4 = D2;
      D2 = C3;
      C3 = rotl(B2, 30);
      B2 = A2;
...
var MD5 = class extends HashMD {
  ...
  get() {
    const { A: A2, B: B2, C: C3, D: D2 } = this;
    return [A2, B2, C3, D2];
  }
  set(A2, B2, C3, D2) {
    this.A = A2 | 0;
...
var idxLR = (() => {
  ...
  for (let i3 = 0; i3 < 4; i3++)
    for (let j4 of res)
      j4.push(j4[i3].map((k4) => Rho160[k4]));
  return res;
})();
var idxL = (() => idxLR[0])();
var idxR = (() => idxLR[1])();
...
function ripemd_f(group, x4, y3, z3) {
  if (group === 0)
    return x4 ^ y3 ^ z3;
  if (group === 1)
    return x4 & y3 | ~x4 & z3;
...
function createClient(parameters) {
  const { batch, chain: chain2, ccipRead, key = "base", name: name2 = "Base Client", type = "base" } = parameters;
...
async function sendCalls(client, parameters) {
  const { account: account_ = client.account, capabilities, chain: chain2 = client.chain, experimental_fallback, experimental_fallbackDelay = 32, forceAtomic = false, id, version: version2 = "2.0.0" } = parameters;
  const account = account_ ? parseAccount(account_) : null;
...
async function getEnsResolver(client, parameters) {
  ...
  const tlds = chain2 == null ? void 0 : chain2.ensTlds;
  if (tlds && !tlds.some((tld) => name2.endsWith(tld)))
    throw new Error(`${name2} is not a valid ENS TLD (${tlds == null ? void 0 : tlds.join(", ")}) for chain "${chain2.name}" (id: ${chain2.id}).`);
...
async function multicall(client, parameters) {
  ...
  for (let i3 = 0; i3 < aggregate3Results.length; i3++) {
    ...
    if (result.status === "rejected") {
      if (!allowFailure)
        throw result.reason;
      for (let j4 = 0; j4 < chunkedCalls[i3].length; j4++) {
        results.push({
          status: "failure",
          error: result.reason,
          result: void 0
    ...
    for (let j4 = 0; j4 < aggregate3Result.length; j4++) {
      const { returnData, success } = aggregate3Result[j4];
      const { callData } = chunkedCalls[i3][j4];
...
async function simulateBlocks(client, parameters) {
  ...
  try {
    ...
    return result.map((block2, i3) => ({
      ...formatBlock(block2),
      calls: block2.calls.map((call2, j4) => {
        var _a, _b;
        const { abi: abi2, args, functionName, to: to2 } = blocks[i3].calls[j4];
  ...
  constructor(x4, y3) {
    super("Found ambiguous types in overloaded ABI Items.", {
      metaMessages: [
        ...
        `\`${x4.type}\` in \`${normalizeSignature(formatAbiItem(x4.abiItem))}\`, and`,
...
async function simulateCalls(client, parameters) {
  ...
  const decimals = ((block_decimals == null ? void 0 : block_decimals.calls) ?? []).map((x4) => x4.status === "success" ? x4.result : null);
  const symbols = ((block_symbols == null ? void 0 : block_symbols.calls) ?? []).map((x4) => x4.status === "success" ? x4.result : null);
  const tokenURI = ((block_tokenURI == null ? void 0 : block_tokenURI.calls) ?? []).map((x4) => x4.status === "success" ? x4.result : null);
  const changes = [];
...
var __spreadArray = function(to2, from8, pack) {
  if (pack || arguments.length === 2)
    for (var i3 = 0, l6 = from8.length, ar2; i3 < l6; i3++) {
      if (ar2 || !(i3 in from8)) {
        if (!ar2)
          ar2 = Array.prototype.slice.call(from8, 0, i3);
        ar2[i3] = from8[i3];
      }
    }
...
function chain(...args) {
  const id = (a2) => a2;
  const wrap = (a2, b3) => (c5) => a2(b3(c5));
  const encode8 = args.map((x4) => x4.encode).reduceRight(wrap, id);
  const decode7 = args.map((x4) => x4.decode).reduce(wrap, id);
  return { encode: encode8, decode: decode7 };
}
...
function normalize(fn) {
  afn(fn);
  return { encode: (from8) => from8, decode: (to2) => fn(to2) };
}
function convertRadix(data, from8, to2) {
  if (from8 < 2)
...
var radix2carry = (from8, to2) => from8 + (to2 - gcd(from8, to2));
var powers = (() => {
  let res = [];
...
function convertRadix2(data, from8, to2, padding2) {
  aArr(data);
  if (from8 <= 0 || from8 > 32)
    throw new Error(`convertRadix2: wrong from=${from8}`);
  if (to2 <= 0 || to2 > 32)
    throw new Error(`convertRadix2: wrong to=${to2}`);
  if (radix2carry(from8, to2) > 32) {
    throw new Error(`convertRadix2: carry overflow from=${from8} to=${to2} carryBits=${radix2carry(from8, to2)}`);
  }
  let carry = 0;
  let pos = 0;
...
function genBech32(encoding) {
  ...
  function encode8(prefix, words, limit = 90) {
...
var baseX = ({ prefix, name: name2, alphabet: alphabet3 }) => {
  const { encode: encode8, decode: decode7 } = base_x_default(alphabet3, name2);
  return from6({
    prefix,
    name: name2,
    encode: encode8,
...
var decode2 = (string, alphabet3, bitsPerChar, name2) => {
  const codes = {};
...
var alphabetBytesToChars = alphabet2.reduce((p3, c5, i3) => {
  p3[i3] = c5;
  return p3;
}, []);
var alphabetCharsToBytes = alphabet2.reduce((p3, c5, i3) => {
...
*/
//# sourceMappingURL=chunk-O5MPSLLY.js.map
